...
cargo clippy --all-features -- -D warnings > src_lint_log.txt 2>&1

cargo test --all-features > src_test_log.txt 2>&1
# cargo test --features "windows-interop,messages"  > src_test_log.txt 2>&1
...
# 包含功能、修复、文档、重构
./scripts/generate-changelog.sh --include feat,fix,docs,refactor

./scripts/generate-changelog.sh --exclude chore,test,build,ci
